	  ...
	  _createClass(_class, [{
	    ...
	    */
	    ...
	    value: function renderControls(data) {
	      ...
	      // start JLN moved section
	      if (!isCollapsed) {
	        if (widgets.muteOrSolo) {
	          headerChildren.push(
		    (0, _h2.default)('div.btn-group', [(0, _h2.default)('button.btn.btn-xs.btn-brown' + muteClass, {
	            attributes: {
	              type: 'button'
	            },
	            onclick: function onclick() {
	          ...
	          }, ['Mute']), (0, _h2.default)('button.btn.btn-xs.btn-seablue' + soloClass, {
	            onclick: function onclick() {
	      ...
	      if (!isCollapsed) {
	      /* ici etait if (widgets.muteOrSolo) { controls.push(...) } que JLN a deplace plus haut 
	      */
	        if (widgets.volume) {
...
/***/ (function(module, exports) {
	...
	/*
	// conversion avec clip (saturation)
	  function floatTo16BitPCM(output, offset, input) {
	    var writeOffset = offset;
	    for (var i = 0; i < input.length; i += 1, writeOffset += 2) {
	      var s = Math.max(-1, Math.min(1, input[i]));
	      output.setInt16(writeOffset, s * 0x7FFF, true);
	    }
	  }
	*/

	// conversion avec normalisation
	  function floatTo16BitACG( output, offset, input ) {
	    var pos = offset;
	    var fmin = 0.0; var fmax = fmin;
	    // peak detect loop
	    for ( var i = 0; i < input.length; i += 1 ) {
		fmin = Math.min( fmin, input[i]);
		fmax = Math.max( fmax, input[i]);
		}
	    fmax = Math.max( fmax, -fmin );
	    // scaling
	    var k = 31653.0/fmax;	// -0.3 dB = 0.966; 0.966 * 0x7FFF = 31653
	    for ( var i = 0; i < input.length; i += 1, pos += 2 ) {
	      output.setInt16( pos, input[i] * k, true);
	    }
	  }
	  ...
	  function encodeWAV(samples) {
	    ...
	    var buffer = new ArrayBuffer(44 + samples.length * 2);	// 44 = taille du header
	    ...
	    floatTo16BitACG(view, 44, samples);
...
/* partie derivee de l'ancien contenu de emitter.js
 * apres eradication de jquery ;-)
 */

///
/// HTML generation for common UI
///
document.getElementById('soft_zone').innerHTML =
`<div class="toolbar">

<div class="btn-group">
  <button type="button" id="b_pause" class="btn-orange btn fas" title="Pause">&#xf04c;</button>
  <button type="button" id="b_play" class="btn-green btn fas" title="Play">&#xf04b;</button>
  <button type="button" id="b_stop" class="btn-red btn fas" title="Stop">&#xf04d;</button>
  <button type="button" id="b_rewind" class="btn-green btn fas" title="Rewind">&#xf049;</button>
</div>

<div class="btn-group">
  <button type="button" id="b_zoomin" class="btn-seablue btn fas" title="Zoom in">&#xf00e;</button>
  <button type="button" id="b_zoomout" class="btn-seablue btn fas" title="Zoom out">&#xf010;</button>
</div>

<div class="btn-group">
  <button type="button" id="b_mixdown" class="btn-black btn fas" title="Mix download">&#xf019;</button>
</div>

</div>

<div id="mixdown_link"></div>
...
<div class="playlist-bottom-bar">
	...
	<div class="red" id="error_msg"></div>
</div>`;

///
/// creation de l'objet dit "playlist" a partir des donnees specifiques du titre playlist_params et playlist_tracklist
///
var playlist_defaults = {
	container: document.getElementById("playlist"),
	waveHeight: 80,		// il fallait 100 avec Mute|Solo, mais on les a depaces a droite du titre
	isAutomaticScroll: true,		// default was false
	timescale: true,			// default was false
	zoomLevels: [2000, 4000, 8000, 16000],	// en avoir plus d'un n'a de sens que si on a les boutons
	samplesPerPixel: 4000,			// initial zoom, une des valeurs de zoomLevels[]
	colors: {
		waveOutlineColor: '#EFF',	// wave background fill
		timeColor: 'grey',		// ticks de la timeline (?)
		},
	controls: {				// left sidebar
		show: true,			// default was false
		width: 200,			// default was bad
		widgets: {			// par defaut tous les widgets sont actifs,
			muteOrSolo: true,	// ici on veut en enlever quelques uns
			volume: true,
			stereoPan: true,
			collapse: false,
			remove: false,
	...
	exclSolo: true,				// default is false
};

// N.B. playlist_params peut overrider des params de playlist_defaults
var playlist = WaveformPlaylist.init( Object.assign( playlist_defaults, playlist_params ) );
// playlist.initExporter();

// charger les mp3 et configurer volumes et pans initiaux
playlist.load( playlist_tracklist );	// .then(function() {  //could do stuff with the playlist. });
...
// transport buttons events
document.getElementById('b_play').onclick = function() { ee.emit("play"); };
document.getElementById('b_pause').onclick = function() { ee.emit("pause"); };
document.getElementById('b_stop').onclick = function() { ee.emit("stop"); };
document.getElementById('b_rewind').onclick = function() { ee.emit("rewind"); };
//zoom buttons
document.getElementById('b_zoomin').onclick = function() { ee.emit("zoomin"); };
document.getElementById('b_zoomout').onclick = function() { ee.emit("zoomout"); };
// mixdown button
playlist.initExporter();	// pour avoir l'exportWorker demarre par startaudiorendering
document.getElementById('b_mixdown').onclick = function()
	{
	document.getElementById('mixdown_link').innerHTML = "mixing... please wait";
	ee.emit('startaudiorendering', 'wav');
	};
// download link
function displayDownloadLink( link ) {
var fnam = document.title + ' - mix.wav';
document.getElementById('mixdown_link').innerHTML =
	'mix ready : <a href="' + link + '" download="' + fnam + '">' + fnam + '</a>';
}
// mixdown callback
...
var timeDiv = document.getElementById('cursor_time');

ee.on("timeupdate", function(time) {
timeDiv.innerHTML = '@3 ' + myTime(time);
});


///
/// progressbar system by JLN
///

// un array indexe par les noms de fichier car on se sait pas dans quel ordre les assets vont apparaitre
// chaque element est un array de 2 nombres : status [0, 3] et percent [0.0, 100.0] 
var progressBars = new Object();

// poor peoples's progress bars en ASCII
var progressStars = '**********';	// 10 caracteres pour 11 niveaux de progression
var progressDiv = document.getElementById('progress');

function displayProgress() {
var keys = Object.keys( progressBars ); // array of the keys
var html = '<p><i>Please wait...</i></p><table class="progress">'; var cnt = 0; var name;
for	( var i in keys )
	{
	name = keys[i];
	cnt = Math.floor( progressBars[name][1] / 10 );
	// html += `<tr><td>${name}</td><td>${progressBars[keys[i]][0]}</td><td><span>${progressStars.substr(0,cnt)}</span></td></tr>`; 
	html += `<tr><td>${name}</td><td><span>${progressStars.substr(0,cnt)}</span></td></tr>`; 
	}
html += '</table>';
...
// les deux callbacks pour loading progress

// cette fonction va recevoir :
//	state : un entier de 0 a 3 signifiant ["uninitialized", "loading", "decoding", "finished"]
//	src   : un objet File dont on peut lire le nom, si on veut
ee.on("audiorequeststatechange", function( state, src )
	{
	var name = src;
	if (src instanceof File) name = src.name;

	if	( progressBars[name] == undefined )
		{
		progressBars[name] = new Array();
		progressBars[name][1] = 0;
		}
	progressBars[name][0] = [state];
	displayProgress();
	// console.log("Track " + name + " is in state " + state );
	} );

// cette fonction va recevoir :
//	percent : un nombre de 0.0 a 100.0
//	src   : un objet File dont on peut lire le nom, si on veut
ee.on("loadprogress", function( percent, src )
	...
	if	( progressBars[name] == undefined )
		{
		progressBars[name] = new Array();
		progressBars[name][0] = 0;
		}
	progressBars[name][1] = percent;
	displayProgress();
	// console.log("Track " + name + " has loaded " + percent + "%");
	} );

// event meaning "Tracks have all finished decoding."
ee.on("audiosourcesloaded", function() {
eraseProgress();
});

// event meaning "Tracks have been rendered"
